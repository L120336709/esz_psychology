...
(function ($) {
    $.extend({
        ...
        table: {
            ...
            importTemp: function () {
                window.location.href= ctx + "achievement/download";
            },
            // 导入数据
            importExcel: function (formId) {
                ...
                layer.open({
                    ...
                    //title: '导入数据',
                    content: $('#' + currentId),
                    //content: '<form id="importForm" name="importFile" class="form-horizontal" method="post" enctype="multipart/form-data"><input id="file" name="file"class="file-loading" type="file" multiple accept=".xls,.xlsx">    </form>',
            ...
            selectDictLabel: function (datas, value) {
                ...
                $.each(datas, function (index, dict) {
                    if (dict.dataValue == ('' + value)) {
                        var listClass = $.common.equals("default", dict.listClass) ? "" : "badge badge-" + dict.listClass;
                        actions.push($.common.sprintf("<span class='%s'>%s</span>", listClass, dict.dataLabel));
                        return false;
                    }
                });
                return actions.join('');
        ...
        operate: {
            ...
            healthView: function (id, width, height) {
                var aaa = $.table;
                debugger;
                var _url = $.common.isEmpty(id) ? $.table._option.detailUrl : $.table._option.detailUrl.replace("{id}", id);
                var _width = $.common.isEmpty(width) ? "800" : width;
            ...
            examineTab: function (id) {
                if ($.common.isEmpty(id) && $.table._option.type == table_type.bootstrapTreeTable) {
                    ...
                    var url = $.table._option.examineUrl.replace("{id}", row[$.table._option.uniqueId]);
                    $.modal.open("审核" + $.table._option.modalName, url);
                } else {
                    $.modal.open("审核" + $.table._option.modalName, $.operate.examineUrl(id));
                }
            },

            // 修改访问地址
            examineUrl: function (id) {
                var url = "/404.html";
                if ($.common.isNotEmpty(id)) {
                    url = $.table._option.examineUrl.replace("{id}", id);
                } else {
                    var id = $.common.isEmpty($.table._option.uniqueId) ? $.table.selectFirstColumns() : $.table.selectColumns($.table._option.uniqueId);
                    if (id.length == 0) {
                        $.modal.alertWarning("请至少选择一条记录");
        ...
        common: {
            //选中checkbox
            checkbox:function(obj,checked){
                $(obj).attr("checked", checked);
                if(checked){
                    $(obj).parents('.icheckbox-blue').addClass("checked");
                }else {
                    $(obj).parents('.icheckbox-blue').removeClass("checked");
                }
            },
            // 判断字符串是否为空
